...
gzip -c bots.ndjson | curl -sS -X POST \
  ...
  -H "Authorization: Bearer $FINSEO_LOG_TOKEN" \
  --data-binary @-
```
...
```bash
*/10 * * * * F=/var/log/nginx/finseo-bots.log; [ -s "$F" ] && mv "$F" "$F.send" && kill -USR1 $(cat /var/run/nginx.pid) && sleep 1 && gzip -c "$F.send" | curl -sS -X POST "https://app.finseo.ai/api/webhooks/server-logs" -H "Authorization: Bearer $FINSEO_LOG_TOKEN" --data-binary @- && rm -f "$F.send"
```
