...
| `get_visibility_timeseries` | Per-day visibility, mentions, ordinal position (`avgMentionOrder`), mention depth and sentiment (same values as the dashboard chart) |
...
| `get_competitor_ranking` | Competitor ranking with the full dashboard KPI set: visibility, mentions, Share of Voice, ordinal position (`avgMentionOrder`), mention depth, #1/Top-3 share, head-to-head and citations — returns the competitor `id` used by the drill-down tools |
